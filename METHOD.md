...
RComPlEx (Comparative analysis of plant co-expression in R) identifies **conserved co-expressed orthologs (co-expressologs)** across multiple species. Unlike simple orthology, this method finds genes that are not only orthologous but also **co-expressed in functionally similar networks** across species.

The key innovation is detecting multi-species **cliques** where ALL gene pairs show significant co-expression, ensuring functional conservation of entire gene modules.
